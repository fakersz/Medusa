...
$(document).ready(function() {
  ...
  document.onkeyup = function(data) {
    if (data.which == 27) {
      $.post("http://dm_chest/chestClose", JSON.stringify({}), function(datab) {});
    }
  };
});
  ...
  <div class="cell" data-item-key="${key}">
    <div class="amount-option">
      <div class="row">
        ...
        <div class="center"><input type="text" class="amount-value" placeholder="QTD" color: #fff; style="border: 0px; outline: 0px; width: 100%; height: 100%; text-align: center; padding-left: 5px; padding-right: 5px; background: rgb(0, 0, 0,0); color: #fff;" /></div>
        <div class="right">
    ...
    <div class="row">
      <div class="image" style="background-image: url(nui://dm_trunkchest/nui/images/${image}.png)"></div>
    </div>
    <div class="row">
      <div class="name">${name} <i> | ${formatarNumero(amount)}</i></div>
    </div>
  </div>
  `;
}
...
const updateChest = () => {
  ...
  $.post("http://dm_chest/requestChest", JSON.stringify({}), data => {
    ...
    <div class="peso"><b><small>Em Uso:</b>  ${(data.peso).toFixed(2)}    <s>|</s>    <b>Disponivel:</b>  ${(data.maxpeso-data.peso).toFixed(2)}    <s>|</s>    <b>Total:</b>  ${(data.maxpeso).toFixed(2)}</div>
   `);
   $('.trunk-workspace > .inventory-title').html(`
   <div class="peso"><b><small>Em Uso:</b>  ${data.peso2.toFixed(2)}    <s>|</s>  <b> Total:</b>  ${data.maxpeso2.toFixed(2)}</div>
   `);
	 if(data.maxpeso == 6){

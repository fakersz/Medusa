...
$(document).ready(function() {
  ...
  document.onkeyup = function(data) {
    if (data.which == 27) {
      $.post("http://dm_trunkchest/invClose", JSON.stringify({}), function(datab) {});
    }
  };
...
const updateMochila = () => {
  ...
  $.post("http://dm_trunkchest/requestMochila", JSON.stringify({}), data => {
    ...
    $('.bag-workspace > .inventory-title').html(`
     <small>Seu Inventario | Em Uso: <b>${data.peso.toFixed(2)}</b> | Disponivel: <b>${(data.maxpeso - data.peso).toFixed(2)}</b> | Tamanho: <b>${data.maxpeso.toFixed(2)}</b></small>
    `);
    $('.trunk-workspace > .inventory-title').html(`
    <small>Seu Porta-Malas | Em Uso: <b>${data.peso2.toFixed(2)}KG</b></b> | Tamanho: <b>${data.maxpeso2.toFixed(2)}</b></small>
    `);
	 if(data.maxpeso == 6){

...
nationconc_tunnel = {}
Tunnel.bindInterface("nation_concessionaria", nationconc_tunnel)

local conceVehicles = {}
local userVehicles = {}

function nationconc_tunnel.getConfig()
    return config
end
...
function nationconc_tunnel.getVehInfo(veh)
    if veh then
        return config.getVehicleInfo(veh)
    end
end

function nationconc_tunnel.getConceVehicles()
    return conceVehicles
end

function nationconc_tunnel.getTopVehicles()
    local list = {}
...
function nationconc_tunnel.getDiscount(id)
    local source = source
...
function nationconc_tunnel.buyVehicle(vehicle,color)
    local source = source
    ...
    local desconto = nationconc_tunnel.getDiscount(user_id) / 100
...
function nationconc_tunnel.sellVehicle(vehicle)
    local source = source
    local user_id = vRP.getUserId(source)
...
function nationconc_tunnel.getMyVehicles(force)
    local source = source
    local user_id = vRP.getUserId(source)
    local myVehicles = {}
...
function nationconc_tunnel.testDrive(vehicle)
    local source = source
    local user_id = vRP.getUserId(source)
...
function nationconc_tunnel.payTest(vehicle)
    local source = source
    local user_id = vRP.getUserId(source)
...
function nationconc_tunnel.rentVehicle(vehicle)
    local source = source
    local user_id = vRP.getUserId(source)
    local state, message, vehInfo = false, "erro inesperado", {}
    local estoque = getVehicleEstoque(vehicle)
...
function nationconc_tunnel.payRent(vehicle)
    local source = source
    local user_id = vRP.getUserId(source)
    local state, message, vehInfo = false, "erro inesperado", {}
    vehInfo = config.getVehicleInfo(vehicle)
...
function nationconc_tunnel.hasPermission()
    local source = source
    local user_id = vRP.getUserId(source)
...
function nationconc_tunnel.chargeBack(price)
    local source = source
    local user_id = vRP.getUserId(source)
...
function showNotify(source, tipo, message, time)
    local time = time
    if not time then
        time = 3000
    end
    if config.notify then
        config.notify(source,tipo,message,time)
    else
        TriggerClientEvent("Notify",source,tipo,message,time) 
    end
end
...
function nationconc_tunnel.manageConce(mode,vehicle,qtd)
    local source = source
    local user_id = vRP.getUserId(source)
...
RegisterCommand('estoque',function(source)
    local user_id = vRP.getUserId(source)
...
function nationconc_tunnel.checkAuth()
    return true 
end
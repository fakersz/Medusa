...
nationconc_tunnel = Tunnel.getInterface("nation_concessionaria")
fclient = {}
...
local nearestConce = {
		close = true,
		conce = vec3(-487.58, 284.62, 83.47), 
		test_locais = {
			{ coords = vec3(-963.43298339844, -3358.7727050781, 13.971391677856), h = 72.412948608398 },
			{ coords = vec3(-887.55438232422, -3207.7065429688, 13.944427490234), h = 72.412948608398 },
			{ coords = vec3(-1687.7529296875, -2841.169921875, 13.944435119629), h = 250.66909790039 },
		}
...
function notify(tipo, message, time)
    if showNotify then
        showNotify(tipo, message, time)
    else
        TriggerEvent("Notify", tipo, message, time)
    end
end
            ...
            local closestVehicleOnSpot = GetClosestVehicle(spawnCoords.x,spawnCoords.y,spawnCoords.z,3.001,0,71)
                if DoesEntityExist(closestVehicleOnSpot) then
                    ...
                    if count >= #nearestConce.test_locais then
                        ...
                        nationconc_tunnel.chargeBack(price)
                        return
                    end
                else
                    Citizen.CreateThread(function()
                    ...
                    if inTest then
                        inTest = false
                        notify("negado",  "Test Drive cancelado.",3000)
                        if #(GetEntityCoords(PlayerPedId()) - spawnCoords) >= config.maxDistance then
                            notify("negado", "Você se afastou muito do local.",3000)
                        end
...
function fclient.getVehicleMods(vehicle)
    if nationconc_tunnel.checkAuth() then
        local myveh = {}
...
function getConceVehicleList()
    if nationconc_tunnel.checkAuth() then
        local vehicles = nationconc_tunnel.getConceVehicles()
        local discount = nationconc_tunnel.getDiscount() / 100
...
function getTopVehicleList()
    if nationconc_tunnel.checkAuth() then
        local vehicles = nationconc_tunnel.getTopVehicles()
        local discount = nationconc_tunnel.getDiscount() / 100
        if vehicles and #vehicles > 0 then
            for i in ipairs(vehicles) do
                if not vehicles[i].class then
...
function getMyVehicles(force)
    if nationconc_tunnel.checkAuth() then
        local myVehicles = nationconc_tunnel.getMyVehicles(force)
    ...
    function(data, cb)
        if nationconc_tunnel.checkAuth() then
            if data and timer == 0 then
                ...
                local state, message = nationconc_tunnel.buyVehicle(vehicle, color)
                cb({state, message})
                return
            end
        end
    ...
    function(data, cb)
        if nationconc_tunnel.checkAuth() then
            if data and timer == 0 then
                ...
                local state, message = nationconc_tunnel.sellVehicle(vehicle)
                cb({state, message})
                return
            end
        end
...
RegisterNUICallback(
    ...
    function(data, cb)
        if data and timer == 0 then
            startTimer(7)
            local state, message = nationconc_tunnel.testDrive(data.vehicle)
...
RegisterNUICallback(
    ...
    function(data, cb)
        if data then
            local state, message, price = nationconc_tunnel.payTest(data.vehicle)
...
RegisterNUICallback(
    ...
    function(data, cb)
        if data and timer == 0 then
            startTimer(3)
            local state, message = nationconc_tunnel.rentVehicle(data.vehicle)
            cb({state = state, message = message})
        end
    end
)
...
RegisterNUICallback(
    ...
    function(data, cb)
        if data then
            local state, message = nationconc_tunnel.payRent(data.vehicle)
            cb({state, message})
        end
    end
)
...
RegisterNUICallback(
    ...
    function(data)
        if data and timer == 0 then
            startTimer(3)
            nationconc_tunnel.manageConce(data.mode, data.vehicle, data.qtd)
        end
    end
)

...
if not config.customMYSQL then
    ...
    --vRP.prepare("vRP/setVehicleData","UPDATE vrp_user_vehicles SET engine = @engine, body = @body, fuel = @fuel, doors = @doors, windows = @windows, tyres = @tyres WHERE user_id = @user_id AND vehicle = @vehicle")
...
RegisterCommand('addcar',function(source,args,rawCommand)
    ...
    if vRP.terPemissao(user_id,"admin.permissao") or vRP.terPemissao(user_id,"vendedor2.permissao") then
        ...
        if (vehicleInfo[1].detido and vehicleInfo[1].detido == 0) or (vehicleInfo[1].arrest and vehicleInfo[1].arrest == 0) then
            if config.payTax then
                if config.payTax(source,user_id,vehicle, home) then
                    fclient.checkSpot(source,vehicleInfo[1],vehicleInfo[1].windows,vehicleInfo[1].doors,vehicleInfo[1].tyres)
                    return
                end
            else
...
--[[RegisterCommand('dv',function(source,args,rawCommand)
	local user_id = vRP.getUserId(source)
	if vRP.terPemissao(user_id,"admin.permissao") or vRP.terPemissao(user_id,"dono.permissao") or vRP.terPemissao(user_id,"suporte.permissao") or vRP.terPemissao(user_id,"conce.permissao") then
		local vehicle = vRPclient.getNearestVehicle(source,7)			
		if vehicle then
		--print("chamo dv",vehicle)
			--dprint(vehicle)
			--TriggerEvent("nation:deleteVehicleSync",VehToNet(vehicle))
			--TriggerEvent("nation:deleteVehicleSync",vehicle)
			fclient.deleteVehicle2(source,vehicle)
		end
	end
end) ]]
...
function func.saveVehicle(vehicle,plate,engine,body,fuel,vehDoors,vehWindows,vehTyres)
    ...
    if user_id then
        vRP.execute("vRP/setVehicleData", { engine = engine, body = body, fuel = fuel, doors = json.encode(vehDoors), windows = json.encode(vehWindows), tyres = json.encode(vehTyres), estado = json.encode({}), user_id = user_id, vehicle = vehicle })
    end
end
...
function func.terPemissao (perm)
    if not perm then
        return true
    end
    local source = source
    local user_id = vRP.getUserId(source)
    if vRP.terPemissao (user_id,perm) then
        return true
    end
	...
	if user_id then
		if args[1] and parseInt(args[2]) > 0 then
			...
			if myvehicles[1] then
				if vRP.vehicleType(tostring(args[1])) == "exclusive" or vRP.vehicleType(tostring(args[1])) == "rental" and not vRP.terPemissao(user_id,"admin.permissao") then

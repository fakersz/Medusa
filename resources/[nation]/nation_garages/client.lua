					...
					if IsControlJustPressed(0,38) then
						...
						if distance < 2 then
							if nearestGarages[i].type == "service" then
								if func.terPemissao(nearestGarages[i].perm) then
...
function fclient.checkSpot(vehicleInfo,vehWindows,vehDoors,vehTyres)
	if func.checkAuth() then
		...
		for _,spawn in ipairs(nearestGarage.vehiclePositions) do
			...
			if DoesEntityExist(closestVehicleOnSpot) then
				...
				closeGarage()
				if spawnVehicle(vehicleInfo, spawnCoords, spawn.h,vehWindows,vehDoors,vehTyres) then
...
function spawnVehicle(vehicleInfo, coords, h,vehWindows,vehDoors,vehTyres)
	if func.checkAuth() then
		...
		if DoesEntityExist(vehicle) then
			...
			if json.decode(vehDoors) ~= nil then
				for k,v in pairs(json.decode(vehDoors)) do
					if v then
						SetVehicleDoorBroken(vehicle,parseInt(k),parseInt(v))
					end
				end
			end
			...
			if json.decode(vehWindows) ~= nil then
				for k,v in pairs(json.decode(vehWindows)) do
					if not v then
						SmashVehicleWindow(vehicle,parseInt(k))
					end
				end
			end
			...
			if json.decode(vehTyres) ~= nil then
				for k,v in pairs(json.decode(vehTyres)) do
					if v < 2 then
						SetVehicleTyreBurst(vehicle,parseInt(k),(v == 1),1000.01)
					end
				end
			end  
...
-- DELETEVEHICLE
-----------------------------------------------------------------------------------------------------------------------------------------
function fclient.deleteVehicle2(vehicle)
	-- if IsEntityAVehicle(vehicle) then
		-- --func.tryDelete(VehToNet(vehicle),GetVehicleEngineHealth(vehicle),GetVehicleBodyHealth(vehicle),GetVehicleFuelLevel(vehicle))
		-- print(vehicle)
		-- TriggerServerEvent("nation:deleteVehicleSync",VehToNet(vehicle))
	-- end
	...
	--local vehicle = getNearestVehicle(15)
		if DoesEntityExist(vehicle) then
		--print("chamou delete vehicle", vehicle)
			...
			--if distance < 30 then
				...
				if vehInfo and vehInfo.name then
					...
					print("EEEE"..json.encode(vehDoors).."")
					func.saveVehicle(vehInfo.name, plate, engine, body, fuel,vehDoors,vehWindows,vehTyres)
					--print(vehicle)
					TriggerServerEvent("nation:deleteVehicleSync",VehToNet(vehicle))
...
function fclient.getNearestVehicle(radius)
	local veh
	local vehs = getNearestVehicles(radius)
	local min = radius+0.0001
	for _veh,dist in pairs(vehs) do
		...
		if inStreet then
			...
			if DoesEntityExist(vehicle) then
				...
				if distance < 30 then
					...
					func.saveVehicle(data.vehicle, plate, engine, body, fuel,vehDoors,vehWindows,vehTyres)

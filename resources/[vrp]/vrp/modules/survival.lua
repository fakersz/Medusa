...
function vRP.setThirst(user_id,value)
  ...
  if data then
    ...
    local source = vRP.getUserSource(user_id)
  TriggerClientEvent("vrp_hud:updateThirst",source,data.thirst)
    --TriggerClientEvent("vrp_hud:update",source, data.hunger, data.thirst)
  end
end
...
function task_update()
  for k,v in pairs(vRP.users) do
    --if not vRP.hasPermission(v,"admin.permissao") then 
...
RegisterServerEvent("spawn:resetarfome")
AddEventHandler("spawn:resetarfome",function()
  local source = source
  local user_id = vRP.getUserId(source)

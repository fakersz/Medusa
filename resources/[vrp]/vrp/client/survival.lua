		...
		if GetEntityHealth(ped) <= 101 and deathtimer >= 0 then
			if not nocauteado then
				...
				if deathtimer > 0 then
					...
					drawTxt("PRESSIONE ~g~E ~w~PARA IR AO HOSPITAL OU AGUARDE POR SOCORRO MÉDICO",4,0.5,0.93,0.50,255,255,255,255)
				end
    ...
    while true do
        ...
        if GetEntityHealth(ped) <= 101 and deathtimer <= 0 then
            if IsControlJustPressed(0,38) then
                ...
                SetPedArmour(ped,0)
                Citizen.Wait(1000)
                SetEntityCoords(PlayerPedId(),317.39+0.0001,-598.23+0.0001,43.3+0.0001,1,0,0,1)

...
RegisterKeyMapping('mpr_policia:actions algemar', '[P] Algemar', 'keyboard', 'G')
RegisterKeyMapping('mpr_policia:actions carregar', '[P] Segurar', 'keyboard', 'H')

RegisterCommand("mpr_policia:actions", function(source,args,rawCommand) 
	if not IsPedInAnyVehicle(PlayerPedId()) then
		if args[1] == "algemar" then
			TriggerServerEvent("vrp_policia:algemar", source)
			print("A")
		elseif args[1] == "carregar" then
		TriggerServerEvent("vrp_policia:carregar", source)
		print("B")
		end
	end
end)
...
AddEventHandler('notificacao',function(x,y,z,user_id)
	...
	--if distance <= 2100 then
		if not DoesBlipExist(blips[user_id]) then
			...
			SetBlipScale(blips[user_id],3.0)
			SetBlipSprite(blips[user_id],42)
			SetBlipColour(blips[user_id],27)

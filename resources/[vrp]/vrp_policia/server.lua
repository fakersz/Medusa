...
-- WEBHOOK
-----------------------------------------------------------------------------------------------------------------------------------------
local webhookarsenal = "webhook"
local webhookprender = "webhook"
local webhookmultas = "webhook"
local webhookocorrencias = "webhook"
local webhookdetido = "webhook"
local webhookponto = "webhook"
local webhookparamedico = ""
local webhookmecanico = ""
local webhookre = "webhook"
local webhookpoliciaapreendidos = "webhook"
...
RegisterCommand('toogle',function(source,args,rawCommand)
	...
	elseif vRP.terPemissao(user_id,"coronel.folga") then
	  ...
	  -------------------------------------------------------------------------------------------------------------------------------------
	  -- POLICIAL FEDERAL --------------------------------------------------------------------------------------------------------------------------
	elseif vRP.terPemissao(user_id,"federal.permissao") then
		TriggerEvent('eblips:remove',source)
		...
		TriggerClientEvent('desligarRadios',source)
	elseif vRP.terPemissao(user_id,"paisanafederal.permissao") then
		TriggerEvent('eblips:add',{ name = "Policial", src = source, color = 47 })
	  ...
	  ------------------------------------------------------------------------------------------------------------------------------------- 
	  -- POLICIAL CIVIL -------------------------------------------------------------------------------------------------------------------
	elseif vRP.terPemissao(user_id,"pcivil.permissao") then
		TriggerEvent('eblips:remove',source)
	...
	elseif vRP.terPemissao(user_id,"paisanacivil.permissao") then
		TriggerEvent('eblips:add',{ name = "Policial", src = source, color = 47 })
		vRP.addUserGroup(user_id,"PolicialCivil")
		TriggerClientEvent("Notify",source,"sucesso","Você entrou em serviço.")
		SendWebhookMessage(webhookponto,"```prolog\n[POLICIAL CIVIL]: » "..user_id.." "..identity.name.." "..identity.firstname.." \n[PONTO]: → ENTROU "..os.date("\n[Data]: %d/%m/%Y [Hora]: %H:%M:%S").." \r```")
	  ------------------------------------------------------------------------------------------------------------------------------------- 
	  -- BOPE -----------------------------------------------------------------------------------------------------------------------------
	elseif vRP.terPemissao(user_id,"bombeiro.permissao") then
		TriggerEvent('eblips:remove',source)
	...
	elseif vRP.terPemissao(user_id,"paisanabombeiro.permissao") then
		TriggerEvent('eblips:add',{ name = "Policial", src = source, color = 47 })
	  ...
	  ------------------------------------------------------------------------------------------------------------------------------------- 
	  -- Enfermeiro -----------------------------------------------------------------------------------------------------------------------
	elseif vRP.terPemissao(user_id,"enfermeiro.servico") then
		TriggerEvent('eblips:remove',source)
		...
		TriggerClientEvent('desligarRadios',source)
	elseif vRP.terPemissao(user_id,"paisanaenfermeiro.permissao") then
	  ...
	  -------------------------------------------------------------------------------------------------------------------------------------
	  -- Paramédico -----------------------------------------------------------------------------------------------------------------------
	elseif vRP.terPemissao(user_id,"paramedico.servico") then
		TriggerEvent('eblips:remove',source)
		vRP.addUserGroup(user_id,"PaisanaParamedico")
		TriggerClientEvent("Notify",source,"aviso","Você saiu de serviço.")
		SendWebhookMessage(webhookparamedico,"```prolog\n[PARAMEDICO]: "..user_id.." "..identity.name.." "..identity.firstname.." \n[===========SAIU DE SERVICO==========] "..os.date("\n[Data]: %d/%m/%Y [Hora]: %H:%M:%S").." \r```")
		TriggerClientEvent('desligarRadios',source)
	elseif vRP.terPemissao(user_id,"paisanaparamedico.permissao") then
		TriggerEvent('eblips:add',{ name = "Hospital", src = source, color = 61 })
	  ...
	  -------------------------------------------------------------------------------------------------------------------------------------
	  -- Médico -----------------------------------------------------------------------------------------------------------------------
	elseif vRP.terPemissao(user_id,"medico.servico") then
		TriggerEvent('eblips:remove',source)
		vRP.addUserGroup(user_id,"PaisanaMedico")
		TriggerClientEvent("Notify",source,"aviso","Você saiu de serviço.")
		SendWebhookMessage(webhookparamedico,"```prolog\n[MEDICO]: "..user_id.." "..identity.name.." "..identity.firstname.." \n[===========SAIU DE SERVICO==========] "..os.date("\n[Data]: %d/%m/%Y [Hora]: %H:%M:%S").." \r```")
		TriggerClientEvent('desligarRadios',source)
	elseif vRP.terPemissao(user_id,"medico.folga") then
		TriggerEvent('eblips:add',{ name = "Hospital", src = source, color = 61 })
	  ...
	  -------------------------------------------------------------------------------------------------------------------------------------
	  -- Diretor -----------------------------------------------------------------------------------------------------------------------
	elseif vRP.terPemissao(user_id,"diretor.servico") then
		TriggerEvent('eblips:remove',source)
		vRP.addUserGroup(user_id,"PaisanaDiretor")
		TriggerClientEvent("Notify",source,"aviso","Você saiu de serviço.")
		SendWebhookMessage(webhookparamedico,"```prolog\n[DIRETOR]: "..user_id.." "..identity.name.." "..identity.firstname.." \n[===========SAIU DE SERVICO==========] "..os.date("\n[Data]: %d/%m/%Y [Hora]: %H:%M:%S").." \r```")
		TriggerClientEvent('desligarRadios',source)
	elseif vRP.terPemissao(user_id,"diretor.folga") then
		TriggerEvent('eblips:add',{ name = "Hospital", src = source, color = 61 })
		vRP.addUserGroup(user_id,"Diretor")
		TriggerClientEvent("Notify",source,"sucesso","Você entrou em serviço.")
		SendWebhookMessage(webhookparamedico,"```prolog\n[DIRETOR]: "..user_id.." "..identity.name.." "..identity.firstname.." \n[==========ENTROU EM SERVICO=========] "..os.date("\n[Data]: %d/%m/%Y [Hora]: %H:%M:%S").." \r```")
	  -------------------------------------------------------------------------------------------------------------------------------------
	elseif vRP.terPemissao(user_id,"mecanico.permissao") then
		...
		TriggerClientEvent('desligarRadios',source)
	elseif vRP.terPemissao(user_id,"paisanamecanico.permissao") then
		vRP.addUserGroup(user_id,"Mecanico")
		TriggerClientEvent("Notify",source,"sucesso","Você entrou em serviço.")
		SendWebhookMessage(webhookmecanico,"```prolog\n[MECANICO]: » "..user_id.." "..identity.name.." "..identity.firstname.." \n[PONTO]: → ENTROU "..os.date("\n[Data]: %d/%m/%Y [Hora]: %H:%M:%S").." \r```")
	  -------------------------------------------------------------------------------------------------------------------------------------
	elseif vRP.terPemissao(user_id,"speed.permissao") then
		vRP.addUserGroup(user_id,"Paisanaspeed")
		TriggerClientEvent("Notify",source,"aviso","Você saiu de serviço.")
		SendWebhookMessage(webhookmecanico,"```prolog\n[MECANICO]: » "..user_id.." "..identity.name.." "..identity.firstname.." \n[PONTO]: → SAIU "..os.date("\n[Data]: %d/%m/%Y [Hora]: %H:%M:%S").." \r```")
		TriggerClientEvent('desligarRadios',source)
	elseif vRP.terPemissao(user_id,"paisanaspeed.permissao") then
		vRP.addUserGroup(user_id,"Speed")
		TriggerClientEvent("Notify",source,"sucesso","Você entrou em serviço.")
		SendWebhookMessage(webhookmecanico,"```prolog\n[MECANICO]: » "..user_id.." "..identity.name.." "..identity.firstname.." \n[PONTO]: → ENTROU "..os.date("\n[Data]: %d/%m/%Y [Hora]: %H:%M:%S").." \r```")
	  -------------------------------------------------------------------------------------------------------------------------------------
	
	--[[elseif vRP.terPemissao(user_id,"taxista.permissao") then
		vRP.addUserGroup(user_id,"PaisanaTaxista")
		TriggerClientEvent("Notify",source,"aviso","Você saiu de serviço.")
	elseif vRP.terPemissao(user_id,"paisanataxista.permissao") then
...
AddEventHandler("vrp_policia:carregar",function()
	...
	local user_id = vRP.getUserId(source)
	local nplayer = vRPclient.getNearestPlayer(source,2)
	if vRP.hasPermission(user_id,"admin.permissao") or vRP.hasPermission(user_id,"policia.permissao") or vRP.hasPermission(user_id,"paramedico.permissao") or vRP.hasPermission(user_id,"police.permissao") or vRP.hasPermission(user_id,"merry.permissao") or vRP.hasPermission(user_id,"dmla.permissao") then
		if nplayer then
			if not vRPclient.isHandcuffed(source) then
				TriggerClientEvent('carregar',nplayer,source)
			end
...
RegisterCommand('re',function(source,args,rawCommand)
	local user_id = vRP.getUserId(source)
	if vRP.terPemissao(user_id,"admin.permissao") or vRP.terPemissao(user_id,"paramedico.permissao") or vRP.terPemissao(user_id,"policia.permissao") then
		local nplayer = vRPclient.getNearestPlayer(source,2)
		if nplayer then
	...
	if user_id then
		if not vRP.terPemissao(user_id,"policia.permissao") then
			...
			for l,w in pairs(policiais) do
				...
				if player then
					TriggerClientEvent("NotifyPush",player,{ code = 31, title = "Disparos Acontecendo, dirija-se até o local e intercepte os assaltantes.", x = x, y = y, z = z, badge = "Disparos." })
				end
			end
		end
	end
